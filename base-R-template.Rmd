...
```{r}
avg_length <- mean(iris$Sepal.Length[iris$Petal.Width > 1])
avg_length
...
```{r}
Counting <- data.frame(V1 = integer(), V2 = integer(), V3 = integer(), V4 = integer())
...
plotNormal <- function(repetitions, mean, sd) {
  ...
  hist(numbers, main = paste("Histogram from Normal(", mean, ",", sd, ") Distribution"), col = "blue")
}
plotNormal(1000, 0, 1)
...
```{r}
set.seed(184)
plotNormalExtra <- function(repetitions, mean, sd, color) {
  numbers <- rnorm(repetitions, mean = mean, sd = sd)
  hist(numbers, main = paste("Histogram from Normal(", mean, ",", sd, ") Distribution"), col = color)
}
plotNormalExtra(1000, 0, 1, "blue")
```
